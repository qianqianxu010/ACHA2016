% DESCRIPTION:
%    This is the code to reproduce the result in the Fig.1
...
i = [1:max(ind),1:max(ind)];  %%%  active sampling
k = [ones(1,max(ind)),-ones(1,max(ind))];
j = [greedy_list(1:max(ind),1);greedy_list(1:max(ind),2)];
d_active = sparse(i,j,k,max(ind),N);
for k = 1:length(p0)
     L = d_active(1:ind(k),:)'*d_active(1:ind(k),:);
    dg_act(k) = min(sum(abs(d_active(1:ind(k),:))))/log(N)/p0(k);
    [V,D] = eigs(L,[],2,'SA');
    lamda_active(k)=D(2,2)/log(N)/p0(k);
end

plot(p0,mean_lambda_with,'b',p0,mean_lambda_without,'r',p0,lamda_active,'c','LineWidth',2);
hold on;
plot(p0,mean_dg_with,'b.',p0,mean_dg_without,'r.',p0,dg_act,'c.','LineWidth',2)
ezplot('x-1-y*x*(1-log(y))',[1,max(p0),0,1]);
plot(p0, 1-sqrt(2./p0.*(1-2/N)),'y');
...
legend('Random sampling with replacement','Random sampling without replacement','Greedy sampling',...
    'Minimal degree of with replacement', 'Minimal degree of without replacement', 'Minimal degree of active sampling',...
    'a(p_0)','a_1(p_0)','a_2(p_0)','fontsize',20);